...
!!! tip "Tulevaisuus?"
    ...
    Transformer-arkkitehtuuri dominoi kielimalleissa tällä hetkellä, mutta kehitys ei luonnollisesti pysähdy koskaan. Transformerit vaativat O(n^2) laskentaa kontekstin koon suhteen: jos nostat 200k tokenin kontekstin 400k:iin, laskenta nelinkertaistuu. KV-cache, joka säilöö $K$ ja $V$ vektorit per token, vaatii O(n) muistia, mikä johtaa herkästi kymmenien gigojen muistintarpeeseen. Näitä skaalautuushaasteita pyritään luonnollisesti ratkaista. Vaihtoehtoja on jo näköpiirissä, kuten Discord-yhteisöiden kehittämä [RWKV-7 "Goose" with Expressive Dynamic State Evolution](https://arxiv.org/abs/2503.14456) tai Googlen oma Transformer-haastaja [Titans: Learning to Memorize at Test Time](https://arxiv.org/abs/2501.00663).  RWKV-7:sta löytyy helppolukuinen artikkeli, joka on tosin maksumuurin takana: [A Discord Community Beat Meta’s LLaMA. The Secret? An Architecture From 1960](https://ai.gopubby.com/rwkv-7-beats-llama-3-2-rnn-constant-memory-46064bbf1f64).
...
??? "Tarkemmat laskelmat (klikkaa auki)"
    ...
    **Candidate 2**
    ...
    BLEU & = 0.8825 \cdot \exp\left( \frac{1}{4} (\log \frac{5}{8} + \log \frac{3}{7} + \log \frac{2}{6} + \log \frac{1}{5}) \right) \\
    BLEU & = 0.8825 \cdot \exp\left(\frac{1}{4} (-0.4700 - 0.8472 -1.0986 - 1.6094) \right) \\
    BLEU & = 0.8825 \cdot 0.3655 = 0.323
    \end{align*}
    $$
