...
def _(mo):
    ...
    - **Original source**: https://docs.pytorch.org/tutorials/beginner/introyt/tensors_deeper_tutorial.html
    - **License**: BSD 3-Clause License
    - **Copyright**: (c) 2017-2022, PyTorch contributors
    - **Modifications**:
        - stylistic changes
        - addition of MPS (Apple Silicon) GPU support
        - make whole Notebook runnable without raising errors on cells (see `this_should_raise_error` context manager)
        - converted to Marimo
    ...
    Note: The following cell will allow us to run code that raises errors without stopping the execution of the whole notebook.
    """)
    return
...
def _():
    ...
    def this_should_raise_error():
        """Context manager to catch and display RuntimeErrors cleanly"""
        try:
            yield
    ...
    # Demo
    with this_should_raise_error():
        print(1 / 0)
    return (this_should_raise_error,)


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""
    Copyright notices stop and the actual Notebook follows after this line.
...
def _(torch):
    ...
    empty_like_x = torch.empty_like(x2)
    print(empty_like_x.shape)
    print(empty_like_x)

    zeros_like_x = torch.zeros_like(x2)
    print(zeros_like_x.shape)
    print(zeros_like_x)

    ones_like_x = torch.ones_like(x2)
    print(ones_like_x.shape)
    print(ones_like_x)

    rand_like_x = torch.rand_like(x2)
...
def _(torch):
    ...
    print(fours)
    print(sqrt2s)
    return fours, ones2, threes, twos
...
@app.cell
def _(fours, ones2, threes, torch, twos):
...
@app.cell
def _(this_should_raise_error, torch):
    a2 = torch.rand(2, 3)
    b2 = torch.rand(3, 2)
...
@app.cell
def _(torch):
    rand_bc = torch.rand(2, 4)
    doubled = rand_bc * (torch.ones(1, 4) * 2)
...
def _(torch):
    ...
    b3 = a3 * torch.rand(   3, 2) # 3rd & 2nd dims identical to a, dim 1 absent
    print(b3)

    c3 = a3 * torch.rand(   3, 1) # 3rd dim = 1, 2nd dim identical to a
    print(c3)

    d3 = a3 * torch.rand(   1, 2) # 3rd dim identical to a, 2nd dim = 1
    print(d3)
    return (a3,)
...
@app.cell
def _(a3, this_should_raise_error, torch):
    with this_should_raise_error():
        b_fail = a3 * torch.rand(4, 3) # dimensions must match last-to-first

    with this_should_raise_error():
        c_fail = a3 * torch.rand(2, 3) # both 3rd & 2nd dims different

    with this_should_raise_error():
        d_fail = a3 * torch.rand((0, ))   # can't broadcast with an empty tensor
    return
...
def _(math, torch):
    # common functions
    a4 = torch.rand(2, 4) * 2 - 1
    print('Common functions:')
    print(torch.abs(a4))
    print(torch.ceil(a4))
    print(torch.floor(a4))
    print(torch.clamp(a4, -0.5, 0.5))
    ...
    # bitwise operations
    print('\nBitwise XOR:')
    b4 = torch.tensor([1, 5, 11])
    c4 = torch.tensor([2, 7, 10])
    print(torch.bitwise_xor(b4, c4))

    # comparisons:
    print('\nBroadcasted, element-wise equality comparison:')
    d4 = torch.tensor([[1., 2.], [3., 4.]])
    e4 = torch.ones(1, 2)  # many comparison ops support broadcasting!
    print(torch.eq(d4, e4)) # returns a tensor of type bool

    # reductions:
    print('\nReduction ops:')
    print(torch.max(d4))        # returns a single-element tensor
    print(torch.max(d4).item()) # extracts the value from the returned tensor
    print(torch.mean(d4))       # average
    print(torch.std(d4))        # standard deviation
    print(torch.prod(d4))       # product of all numbers
    ...
    print('\nVectors & Matrices:')
    print(torch.cross(v2, v1, dim=0)) # negative of z unit vector (v1 x v2 == -v2 x v1)
    print(m1)
...
@app.cell
def _(math, torch):
    a5 = torch.tensor([0, math.pi / 4, math.pi / 2, 3 * math.pi / 4])
    print('a:')
    print(a5)
    print(torch.sin(a5))   # this operation creates a new tensor in memory
    print(a5)              # a has not changed

    b5 = torch.tensor([0, math.pi / 4, math.pi / 2, 3 * math.pi / 4])
    print('\nb:')
    print(b5)
    print(torch.sin_(b5))  # note the underscore
    print(b5)              # b has changed
    return
...
def _(torch):
    ...
    print(c7)
    d7 = torch.matmul(a7, b7, out=c7)
    print(c7)                # contents of c have changed

    assert c7 is d7           # test c & d are same object, not just containing equal values
    assert id(c7), old_id    # make sure that our new c is the same object as the old one

    torch.rand(2, 2, out=c7) # works for creation too!
    print(c7)                # c has changed again
    assert id(c7), old_id    # still the same object!
    return
...
def _(torch):
    ...
    a8[0][1] = 561  # we change a...
    print(b8)       # ...and b is also altered
    return
...
def _(torch):
    ...
    assert b9 is not a9      # different objects in memory...
    print(torch.eq(a9, b9))  # ...but still with the same contents!

    a9[0][1] = 561          # a changes...
    print(b9)               # ...but b is still all ones
    return
...
@app.cell
def _(torch):
    a10 = torch.rand(2, 2, requires_grad=True) # turn on autograd
    print(a10)

    b10 = a10.clone()
    print(b10)

    c10 = a10.detach().clone()
    print(c10)
...
def _(torch):
    if torch.cuda.is_available():
        print('We have a CUDA GPU!')
    elif torch.backends.mps.is_available():
        print('We have an Apple Silicon GPU!')
    else:
...
def _(torch):
    ...
    x_dev = torch.rand(2, 2, device=my_device)
    print(x_dev)
    return (my_device,)
...
@app.cell
def _(my_device, torch):
    y_dev = torch.rand(2, 2)
    y_dev = y_dev.to(my_device)
    return
...
@app.cell
def _(torch):
    a_sq = torch.rand(3, 226, 226)
    b_sq = a_sq.unsqueeze(0)
...
@app.cell
def _(torch):
    a_sq2 = torch.rand(1, 20)
    print(a_sq2.shape)
    print(a_sq2)

    b_sq2 = a_sq2.squeeze(0)
    print(b_sq2.shape)
    print(b_sq2)

    c_sq2 = torch.rand(2, 2)
    print(c_sq2.shape)

    d_sq2 = c_sq2.squeeze(0)
    print(d_sq2.shape)
    return
...
@app.cell
def _(torch):
    a_us = torch.ones(4, 3, 2)
    b_us = torch.rand(   3)     # trying to multiply a * b will give a runtime error
    c_us = b_us.unsqueeze(1)       # change to a 2-dimensional tensor, adding new dim at the end
    print(c_us.shape)
    print(a_us * c_us)             # broadcasting works again!
    return
...
def _(torch):
    ...
    return numpy_array, pytorch_tensor
...
def _(torch):
    ...
    print(numpy_rand)
    return numpy_rand, pytorch_rand
...
@app.cell
def _(numpy_array, numpy_rand, pytorch_rand, pytorch_tensor):
    numpy_array[1, 1] = 23
    print(pytorch_tensor)
